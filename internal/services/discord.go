...
// SendReportNotification posts a new report. transfer is set when the
// report came from a received transfer, nil for a link-share report.
func (d *Discord) SendReportNotification(file *models.File, reporterIP string, reportCount int, transfer *models.Transfer) error {
	if !d.IsConfigured() {
		return nil
	}
	...
	if transfer != nil {
		// Transferred files are only reachable by their recipients, so a
		// share link is of no use here; name both parties instead.
		title = "Transferred File Reported"
		fields = append(fields,
			models.Field{Name: "Source", Value: "Transfer", Inline: true},
			models.Field{Name: "Sender User ID", Value: fmt.Sprintf("`%d`", transfer.SenderCNSUserID), Inline: true},
			models.Field{Name: "Reporter (Recipient) User ID", Value: fmt.Sprintf("`%d`", transfer.RecipientCNSUserID), Inline: true},
		)
	} else {
		fields = append(fields, models.Field{
			Name:   "Share Link",
			Value:  fmt.Sprintf("%s/shared/%s", d.baseURL, file.ID),
			Inline: false,
		})
	}
